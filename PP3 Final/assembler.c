 ...
 * Modified by: Andrew Parsons, Sivhaun Sera (10 May 2017)
 *      Added pass2()
 */
...
int main (int argc, char * argv[])
{
    ...
    LabelTable table;
    const char * ERROR1 = "Error: Cannot open file \e[33m%s\e[0m.\n";

    /* Process command-line arguments (if any). */
    ptr_file = process_arguments(argc, argv);
    if ( ptr_file == NULL )
    {
    	printError("\e[91m%s\e[0m","FATAL ERROR");
    ...
    table = pass1(ptr_file);
    rewind (ptr_file);

    /* if debug on, print the labels */
    if ( debug_is_on() )
    {
        printLabels(&table);
    }
    ...
    printDebug("\e[33m%s\e[0m\n", "Pass 2");

    pass2(ptr_file, table);

    /* close the file */
    (void) fclose(ptr_file);
...
static FILE * process_arguments(int argc, char * argv[])
{
    ...
    return ptr_file;   /* Everything was OK! */
}
